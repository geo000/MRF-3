...
OLGA_INLINE void GCoptimization::addterm2_checked(EnergyT* e, VarID i, VarID j, EnergyTermType e00, EnergyTermType e01, EnergyTermType e10, EnergyTermType e11, EnergyTermType w)
{
	...
	if ((e00 + e11) - (e01 + e10) > (EnergyTermType)(0.01f))
	...
	try 
	{
		if ( max_num_iterations == -1 )
		{
			...
			do
			{
				...
				printf("iteration = %d  , new energy = %d \n",iter, new_energy);
				iter++;
		...
		else
		{
			...
			{
				printf("cycle  =%d  , energy = %lf ,datacost = %lf , smoothcost = %lf\n",cycle,new_energy,giveDataEnergy(),giveSmoothEnergy());
				gcoclock_t ticks0 = gcoclock();
						...
						:GCoptimization(width*height,num_labels)
{
	...
	setupNeighbData(1,m_height-1,1,m_width-1,4,indexes);// main body contains 4 directions' neighbors
