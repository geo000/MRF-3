...
bool GraphCut4MRF::initGraph(void)
{
	...
	// step 2:  calculate minimum distance of residual points to scribble points
	
	//if (TK::tk_is_file_existed("./subtotal/m_labels.txt") && TK::tk_is_file_existed("./subtotal/m_labels_minimum_energy.txt"))
	//{

	//	TK::tk_elicit(m_labels, m_height,m_width,"./subtotal/m_labels.txt");
	//	TK::tk_elicit(m_labels_minimum_energy, m_height, m_width, "./subtotal/m_labels_minimum_energy.txt");
	//}
	//else
	//{
	...
	//	//for (int i = 0; i < m_labels_num; ++i)
	//	//{
	//	//	int len = m_points[i].size();

	//	//	for (int j = 0; j < len; ++j)
	//	//	{
	//	//		int y_p = m_points[i][j].x;
	//	//		int x_p = m_points[i][j].y;

	//	//		for (int k = 0; k < m_numpixels; ++k)
	//	//		{
	//	//			int x, y;
	//	//			decode(x, y, k);

	//	//			GCoptimization::EnergyTermType label_energy = (x_p - x)*(x_p - x) + (y_p - y)*(y_p - y);

	//	//			if (m_labels_minimum_energy[y][x] > label_energy)
	//	//			{
	//	//				m_labels[y][x] = i;

	//	//				m_labels_minimum_energy[y][x] = label_energy;
	//	//								
	//	//			}
	//	//			
	//	//		}
	...
	//	//TK::tk_dump(m_labels, m_height, m_width, "./subtotal/m_labels.txt");
	//	//TK::tk_dump(m_labels_minimum_energy, m_height, m_width, "./subtotal/m_labels_minimum_energy.txt");
...
void GraphCut4MRF::showResult(bool saveimage)
{
	...
	if (saveimage)
		TK::tk_save_img(m_result, "./subtotal/Initial.png");
