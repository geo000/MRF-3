...
GraphCut4MRF::~GraphCut4MRF()
{
	...
	if (!m_pointMask.empty())    m_pointMask.clear();


	if (!m_labels.empty()) m_labels.clear();
	...
	if (!m_result.empty()) m_result.release();
	if (!m_points.empty()) m_points.clear(); 
	if (!m_slicPoints.empty()) m_slicPoints.clear();
...
bool GraphCut4MRF::initGraph(void)
{
	...
	//check for exists
	if (TK::tk_is_file_existed((m_subtotal + "/m_points.txt").c_str()) && TK::tk_is_file_existed((m_subtotal + "/m_slicPoints.txt").c_str()))
	{

		TK::tk_elicit_points(m_points, (m_subtotal + "/m_points.txt").c_str());
		TK::tk_elicit_points(m_slicPoints, (m_subtotal+ "/m_slicPoints.txt").c_str());
	...
	else
	{
		...
		TK::tk_dump_points(m_points, (m_subtotal+ "/m_points.txt").c_str());
		TK::tk_dump_points(m_slicPoints, (m_subtotal+ "/m_slicPoints.txt").c_str());
...
void GraphCut4MRF::initGraph(const std::string & filename)
{
...
void GraphCut4MRF::graphCutOptimization(int max_iter, std::string tag, SolverMethod solver)
{
	try{
		...
		for (int s = 0; s < m_numpixels; ++s)
		{
			...
			decode(x, y, s);
			m_labels[y][x] = label;
			m_result.at<cv::Vec3b>(y, x) = m_source[label].at<cv::Vec3b>(y, x);
...
{
	//if (!m_gc_controller)  delete  m_gc_controller;

	for (int i = 0; i < m_labels_num; ++i){
		if (!m_unarycost[i]) delete m_unarycost[i];
	}

}

bool MyDataCostFunctor::initUnaryCost(std::string& dumpfolder)
{
	if (m_gc_controller != NULL)
	{
		

		if (TK::tk_is_file_existed((dumpfolder+ "/m_unarycost.txt").c_str()))
		{			
			TK::tk_elicit_malloc(m_unarycost, m_gc_controller->m_labels_num, m_gc_controller->m_numpixels, (dumpfolder+ "/m_unarycost.txt").c_str());
			return true;
		}
		else
		{
			...
			//dump 
			TK::tk_dump_malloc(m_unarycost, m_gc_controller->m_labels_num, m_gc_controller->m_numpixels, (dumpfolder+ "/m_unarycost.txt").c_str());
